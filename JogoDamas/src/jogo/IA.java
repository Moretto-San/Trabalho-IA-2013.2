...
public class IA {
	...
	public Botao[][] result(Action acao, Botao[][] tabuleiroIn) {
		...
		if (acao.getOrigem().getBackground() == Color.GREEN) {
			comerPeca(procurarPecaEscolhidaParaComer(acao.getDestino(), tabuleiro), acao.getOrigem(), acao.getDestino(), tabuleiro);
		}
	...
	public void comerPeca(Botao btnLooser, Botao btnWinner, Botao btnPosicao, Botao[][] tabuleiro) {
	...
	// Esse metódo pinta de vermelho as posições válidas em que uma peça pode se
				...
				if (tabuleiro[btn.getI() - 1][btn.getJ() + 1].getCor() != Cor.VAZIO) {
					if (btn.getCor() != tabuleiro[btn.getI() - 1][btn.getJ() + 1]
						...
						botoesASeremComidos.add(tabuleiro[btn.getI() - 1][btn
								.getJ() + 1]);
						index++;
						mostrarOpcoesParaComer(botoesASeremComidos.get(index), btn, tabuleiro);
					}
				}
			}
		...
		public void mostrarOpcoesParaComer(Botao btnLooser, Botao btnWinner, Botao[][] tabuleiro) {
			// Posição acima à direita
