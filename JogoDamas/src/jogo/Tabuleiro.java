...
public class Tabuleiro extends JPanel{
	...
	private ImageIcon icon1 = new ImageIcon(Tabuleiro.class.getResource("/img/circle-blue.png")); //Peça Azul
	private ImageIcon icon2 = new ImageIcon(Tabuleiro.class.getResource("/img/circle-red.png"));  //Peça Vermelha
	...
	private void carregarTabuleiro(){
		for (int i = 0; i < tabuleiro.length; i++) {
			for (int j = 0; j < tabuleiro[i].length; j++) {
				...
				//Pinta o tabuleiro alternadamente de branco (ficando a grid interpolada entre branco e cinza)
				if(i % 2 == 0){
					if(j % 2 == 0){
	...
	private void HandleButtonEvent(Botao btn){
		if(!suaVezDeJogar){
			JOptionPane.showMessageDialog(null, "Não é a sua vez de jogar!", "Atenção!", JOptionPane.INFORMATION_MESSAGE);
			return;
		}
	...
	//Esse metódo pinta de vermelho as posições válidas em que uma peça pode se mover.
	...
	public void mostrarOpcoes(Botao btn){		
		...
		if(topRight(btn)){
			if(!tabuleiro[btn.getI()-1][btn.getJ()+1].isOcupado()){
				tabuleiro[btn.getI()-1][btn.getJ()+1].setBackground(Color.RED);
			}
			if(tabuleiro[btn.getI()-1][btn.getJ()+1].getCor() != Cor.VAZIO){
				if(btn.getCor() != tabuleiro[btn.getI()-1][btn.getJ()+1].getCor()){
					botoesASeremComidos.add(tabuleiro[btn.getI()-1][btn.getJ()+1]);
					index++;
	...
	//Esse método pinta de vermelho as opções em que a peça poderá comer uma peça adversária
	public void mostrarOpcoesParaComer(Botao btnLooser, Botao btnWinner){		
		//Posição acima à direita
		if(topRight(btnLooser)){
			if(!tabuleiro[btnLooser.getI()-1][btnLooser.getJ()+1].isOcupado()){	
				if(btnWinner.getI() != tabuleiro[btnLooser.getI()-1][btnLooser.getJ()+1].getI() &&
					btnWinner.getJ() != tabuleiro[btnLooser.getI()-1][btnLooser.getJ()+1].getJ()){
					tabuleiro[btnLooser.getI()-1][btnLooser.getJ()+1].setBackground(Color.GREEN);
	...
	public void comerPeca(Botao btnLooser, Botao btnWinner, Botao btnPosicao){
		...
		tabuleiro[btnPosicao.getI()][btnPosicao.getJ()].setBackground(Color.LIGHT_GRAY);
	}
	
	//Esse método serve para encontrar a peça do adversário que o jogador escolheu para comer.
	//Para isso ele sempre deve receber como parâmetro o botão com fundo verde (ou seja, aquele que o jogador escolheu para 
	...
	public void moverPeca(Botao btnOrigem, Botao btnDestino){
		...
		if(btnOrigem.getCor() == Cor.AZUL){
			...
			tabuleiro[btnDestino.getI()][btnDestino.getJ()].setCor(Cor.VERMELHA);
		}
	...
	public boolean topRight(Botao btn){
		return (btn.getI()-1 >= 0 && btn.getI()-1 < 8) && (btn.getJ()+1 >= 0 && btn.getJ()+1 < 8);
	}
